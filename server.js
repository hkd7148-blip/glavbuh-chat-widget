...
import fs from 'fs';
// Если Node < 18, можно использовать node-fetch:
...
function splitIntoChunks(text, chunkSize = 1000, overlap = 200) {
  ...
  for (const sentence of sentences) {
    ...
    if (currentSize + sentenceSize > chunkSize && currentChunk.length > 0) {
      ...
      // Overlap - берем последние слова для связности
...
app.post('/api/admin/users/:email/activate', adminRequired, (req, res) => {
  ...
  res.json({
    ...
    accessFrom: new Date(account.activatedAt || (userStats.get(email)?.registeredAt ?? Date.now())).toISOString().split('T')[0],
    accessTo: new Date(account.expiresAt).toISOString().split('T')[0]
  });
});

// Изменение срока доступа пользователя
app.post('/api/admin/users/:email/extend', adminRequired, (req, res) => {
  const { email } = req.params;
...
app.post('/api/admin/users/:email/block', adminRequired, (req, res) => {
  const { email } = req.params;
...
// Загрузка файлов (временное хранение + обезличивание)
app.post('/api/upload', authRequired, trackUserActivity, upload.single('file'), async (req, res) => {
  try {
    ...
    // Используем единую функцию редактирования PII
...
// Обычный чат с RAG (прокси потоков)
app.post('/api/chat', authRequired, trackUserActivity, async (req, res) => {
  try {
...
// Облегченный чат (SSE-обёртка)
app.post('/api/chat_plus', authRequired, trackUserActivity, async (req, res) => {
  try {
...
// Обезличивание файла (скачивание)
...
app.post('/api/register/init', async (req, res) => {
  try {
    ...
    const userType = String(req.body?.userType || 'individual').trim();
...
app.post('/api/register/verify', (req, res) => {
  try {
    ...
    accounts.set(email, {
      ...
      name: rec.name,
      phone: rec.phone,
      userType: rec.userType || 'individual',
      isActive: true,
...
// Простой endpoint для получения админского токена по секрету (GET — лучше заменить на POST в проде)
...
// Быстрая регистрация тестового пользователя
app.post('/api/quick-register', async (req, res) => {
  try {
    ...
    accounts.set(email, { expiresAt, token, name, phone, isActive: true, activatedAt: Date.now(), activatedBy: 'system' });
    tokens.set(token, { email, expiresAt });
...
// Поиск в базе знаний
app.post('/api/knowledge/search', adminRequired, async (req, res) => {
  try {
